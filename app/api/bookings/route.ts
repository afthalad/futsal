...
export async function POST(request: NextRequest) {
  try {
    const { groundId, customerName, customerPhone, date, startTime, endTime, reason } = await request.json()

    if (!groundId || !customerName || !customerPhone || !date || !startTime || !endTime) {
    ...
    const existingBooking = activeBookings.find(booking => 
      booking.date === date &&
